...
public class Kcoinclient {                      
    ...
    public Kcoinclient() {                 // 구성자에서 초기화 시킴
		        	        ...
		        	        try (Socket socket = new Socket(hostname, port)) {
		        		         ...
		        		         System.out.println("몇번째 블락:" + index);
		        		   	     ...
		        		   	     for (int i = 0; i < 1000; i++) {  //1억을 Block에 할당하고 하나씩 Reset 함.
		        	                 ...
		        	                 block[index].tran_list[i] =  Kcoinp2pserver.tran_list0[i];    // - 수락 전  + 수락 함                                                         //  System.out.format("%d = %s%n", i, wallet[i]); // 출력하기   	      
		         	                 //block에 Time stamps 추가
		        	                 
		        	                 // Kcoinp2pserver.tran_list0[i].reset();  // java 가 다르면 접근 불가
		        		   	     }
...
class transaction implements Serializable{         // 직열화 전송으 위해서 필요함.
